...
export default function YouTubePage() {
  ...
  // Nightly batch — queues one pipeline job per artist at the scheduled time
  const runNightlyBatch = async () => {
    for (const persona of PERSONAS) {
      ...
      try {
        await supabase.from("pipeline_jobs").insert(basePayload);
      } catch {}
    }
  };
        ...
        {activeTab === "pipeline" && (
          <div className="holo-card mb-6 rounded-xl border border-green-500/20 bg-black/60">
            ...
            <div className="p-6 space-y-4">
              <p className="text-xs text-green-700 font-mono">Queues one track job per artist at the set time. All 7 artists run overnight and land in your approval queue.</p>
              ...
              <div className="flex gap-3 items-end">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 text-sm font-mono rounded-lg bg-black/60 border border-green-500/20 text-green-300 focus:outline-none focus:border-green-400/50"
                  />
                </div>
                <button
                  onClick={() => {
                  ...
                  className="px-4 py-2.5 rounded-lg bg-black/60 border border-green-500/20 text-green-600 text-sm font-mono font-medium hover:border-green-400/40 hover:text-green-400 transition-all flex-shrink-0"
                >
                  Set Time
                </button>
                <button
                  onClick={runNightlyBatch}
                  className="px-4 py-2.5 rounded-lg bg-green-500/10 border border-green-400/30 text-green-400 text-sm font-mono font-medium hover:bg-green-500/20 transition-all flex-shrink-0"
                >
                  Run Now
                </button>
              </div>
              ...
              {schedule.length > 0 && (
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-500/5 border border-green-500/15">
                  ...
                  <span className="text-xs text-green-600 font-mono">Scheduled: {schedule[0]} every night</span>
                  {scheduleEnabled && <span className="ml-auto text-xs text-green-400 font-mono glow-text">active</span>}
                </div>
              )}

              {/* Artists that will run */}
              <div>
                <p className="text-xs text-green-800 font-mono uppercase tracking-widest mb-2">Artists included</p>
                <div className="flex flex-wrap gap-2">
                  {PERSONAS.map(p => (
                    <span key={p.name} className="px-2 py-1 rounded-md bg-black/40 border border-green-500/15 text-xs text-green-700 font-mono">{p.name}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-green-500/10">
                ...
                >
                  {autoNext ? "✓ Auto-run next job after manual completion" : "Auto-run next: off"}
                </button>
              </div>
            </div>
          </div>
